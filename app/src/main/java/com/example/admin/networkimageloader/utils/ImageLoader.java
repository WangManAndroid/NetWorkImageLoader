...
public class ImageLoader
{
   ...
   private void initUIHandler(){
		...
		return new Runnable()
		{
			...
			public void run()
			{
				...
				if (isFromNet)
				{
				 ...
				 {
					bm = loadImageFromLocal(new File(path), imageView);
				 }
				// 3、把图片加入到缓存
	...
	private Bitmap loadImageFromLocal(final File path,
...
//				imageSize.height);
		bm=ImageUtil.getScaledBitmap(imageView.getContext().getApplicationContext(), Uri.fromFile(path), imageSize.width, imageSize.height, Bitmap.Config.ARGB_8888);
		return bm;
	}
